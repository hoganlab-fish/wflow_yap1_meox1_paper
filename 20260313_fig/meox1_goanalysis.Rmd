...
- This assumes you ran `meox1_deg_celltype.R` first, in which we merged VECs and LECs into groups for comparison.
...
../Saki_data/Revision_analysis/queue_plots/go
    top15_LEC_GO_pathways_downregulated_generatio_dotplot.pdf
    top15_LEC_GO_pathways_downregulated_geneexpr_dotplot.pdf
    top15_VECs_GO_pathways_downregulated_generatio_dotplot.pdf
    top15_VECs_GO_pathways_downregulated_geneexpr_dotplot.pdf
-->
...
singles_meox_path <- "../analysis/D10051_meox1_dataset/DEGs/Level_03_DEG_mutVSwt_L3_celltype_fc0.00_minpct_0.01.csv"
grouped_meox_path <- "../Revision_analysis/queue_plots/meox1_Level_03_DEG_mutVSwt_L3_celltype_merged_VEC_merged_LEC_fc0.00_minpct_0.01.csv"

singles_meox_hippo_path <- "../analysis/D10051_meox1_dataset/DEGs/analysis/D10051_meox1_dataset/DEGs/Level_03_DEG_hippo_targets_mutVSwt_L3_celltype_fc0.00_minpct_0.00.csv"
grouped_meox_hippo_path <- "../Revision_analysis/queue_plots/meox1_Level_03_DEG_mutVSwt_L3_celltype_merged_VEC_merged_LEC_hippo_targets_fc0.00_minpct_0.00.csv"
...
# cat("\n\nDotplots of gene expression (top 50).\n\n")
...
# 4. sorted genes by variable features (unsupervised, unbiased compared to DGE) and picked the top 50\n
# 5. plotted dotplots on the remainder\n\n

# Attempted heatmaps, not very useful.\n\n")

# cat("\n\n_Suggested figure legend:_ Dotplot shows the expression level for genes from enriched GO terms. Units on the horizontal axis correspond to the genotypes. Point size corresponds to the percent of expression in each cell group, and point colour corresponds to average expression z-scores. Top 50 genes were selected by the most variable genes across the data.\n\n")

# plot gene expr
# plt_name <- "top15_LECs_GO_pathways_downregulated_geneexpr_dotplot"
# plt_data <- DotPlot(object = data, features = ego_lecs_top15_gene_list)$data

# sort and pick top 50 by highly variable genes
# we do this instead of using DEG so its comparable
# hvf_info <- HVFInfo(data)
# ego_lecs_top15_gene_list <- intersect(
#     ego_lecs_top15_gene_list, rownames(HVFInfo(data))
#     )

# ego_lecs_var_metric <- hvf_info[ego_lecs_top15_gene_list, "variance.standardized"]
# names(ego_lecs_var_metric) <- ego_lecs_top15_gene_list
# ego_lecs_top_50_hvg <- names(sort(ego_lecs_var_metric, decreasing = TRUE)[1:50])
...
# plt <- DotPlot(data_lecs, features=ego_lecs_top_50_hvg, 
#     group.by="Genotype", cols=c("#d9d9d9",  "#40004b")
#     ) + 
#     coord_flip() + 
#     theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
# print(plt)
# save_plot_custom(plt, plt_name, height=16, width=4)
...
# head(ego_lec_top15_pathways, n=15)
...
# # plot gene expr
# plt_name <- "top15_LEC_GO_pathways_downregulated_geneexpr_dotplot"
# plt_data <- DotPlot(object = data, features = ego_lec_top15_gene_list)$data

# # sort and pick top 50 by highly variable genes
# # we do this instead of using DEG so its comparable
# hvf_info <- HVFInfo(data)
# ego_lec_top15_gene_list <- intersect(
#     ego_lec_top15_gene_list, rownames(HVFInfo(data))
#     )

# ego_lec_var_metric <- hvf_info[ego_lec_top15_gene_list, "variance.standardized"]
# names(ego_lec_var_metric) <- ego_lec_top15_gene_list
# ego_lec_top_50_hvg <- names(sort(ego_lec_var_metric, decreasing = TRUE)[1:50])
...
# plt <- DotPlot(data_lec, features=ego_lec_top_50_hvg, 
#     group.by="Genotype", cols=c("#d9d9d9",  "#40004b")
#     ) + 
#     coord_flip() + 
#     scale_y_discrete(labels = c("wildtype", "meox1 mutant")) + 
#     theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
# print(plt)
# save_plot_custom(plt, plt_name, height=16, width=4)

# heatmap not very legible sadly
# plt_name <- "top15_LEC_GO_pathways_downregulated_geneexpr_heatmap"
# custom_scaled <- ScaleData(data, features=top15_gene_list)
# plt <- DoHeatmap(data, features=top15_gene_list, group.by="Genotype")
# save_plot_custom(plt, plt_name, height=16, width=8)
...
# head(ego_vecs_top15_pathways, n=15)
...
# plot gene expr
# plt_name <- "top15_VECs_GO_pathways_downregulated_geneexpr_dotplot"
# plt_data <- DotPlot(object = data, features = ego_vecs_top15_gene_list)$data

# # sort and pick top 50 by highly variable genes
# # we do this instead of using DEG so its comparable
# hvf_info <- HVFInfo(data)
# ego_vecs_top15_gene_list <- intersect(
#     ego_vecs_top15_gene_list, rownames(HVFInfo(data))
#     )

# ego_vecs_var_metric <- hvf_info[ego_vecs_top15_gene_list, "variance.standardized"]
# names(ego_vecs_var_metric) <- ego_vecs_top15_gene_list
# ego_vecs_top_50_hvg <- names(sort(ego_vecs_var_metric, decreasing = TRUE)[1:50])
...
# plt <- DotPlot(data_vecs, features=ego_vecs_top_50_hvg, 
#     group.by="Genotype", cols=c("#d9d9d9",  "#40004b")
#     ) + 
#     coord_flip() + 
#     scale_y_discrete(labels = c("wildtype", "meox1 mutant")) + 
