#!/usr/bin/Rscript
# v1 20260317: redo of yap1_dataset_Level_03_hmVEC_volcano_pvalue so that y axis is -log (raw Pval) and x axis is log(expression ratio) and DEGs are coloured (using adjusted P val) -> i)
...
outfile_path_con <- "../Revision_analysis/queue_plots/yap1_dataset_Level_03_hmVEC_volcano_pvalue.continuous.pdf"
outfile_path_dis <- "../Revision_analysis/queue_plots/yap1_dataset_Level_03_hmVEC_volcano_pvalue_RAW.pdf"

hippo_gene_degs <- read_csv(
    "../../Saki_data/analysis/D10025_yap1_dataset/DEGs/Level_03_DEG_hippo_targets_mutVSwt_L3_celltype_fc0.00_minpct_0.00.csv", 
    show_col_types = F
    )
