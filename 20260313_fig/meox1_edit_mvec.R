#!/usr/bin/Rscript
# v1 20260317: redo of meox1_dataset_Level_03_mVEC_volcano_pvalue so that y axis is -log (raw Pval) and x axis is log(expression ratio) and DEGs are coloured (using adjusted P val) -> i)
...
#   meox_dataset_Level_03_mVEC_volcano_pvalue_RAW.pdf
...
outfile_path_con <- "../Revision_analysis/queue_plots/meox1_dataset_Level_03_mVEC_volcano_pvalue.continuous.pdf"
outfile_path_dis <- "../Revision_analysis/queue_plots/meox1_dataset_Level_03_mVEC_volcano_pvalue_RAW.pdf"
...
hippo_mark <- all_degs %>% 
    filter(group == "mVEC" & Gene %in% hippo_targets) %>%
    mutate(sig = ifelse(p_val < 0.05, "DEG (RawPVal<0.05)", "non-DEG"))

# colour="#525252"
plot_discrete <- mvec_data %>%
