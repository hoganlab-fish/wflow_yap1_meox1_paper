...
```{r}
celltype_to_keep <- c("LEC", "VEC", "cycling_LEC_VEC")
...
plot_umap_dataset <- function(seurat){
  ...
  plot_list[[5]] <- DimPlot(seurat,  group.by = c("L2_celltype"), shuffle = T,cols =colours) + ggtitle(seurat@project.name)
  return(plot_list)
...
-> might need to re-run everything and remove those low quality cells.. if that's what they are. 
