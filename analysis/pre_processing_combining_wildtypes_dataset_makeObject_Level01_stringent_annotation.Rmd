...
Combine wildtype from yap1 and meox1 experiments, stringent threshold for meox1.
...
See [here](https://atlassian.petermac.org.au/bitbucket/pages/HL/wflow_yap1_meox1_paper/master/browse/docs/pre_processing_D10051_meox1_dataset_makeObject_Level01_stringent_annotation.html), but the original filtering we performed did not really get rid of all the low quality cells, so we need to introduce a second filtering step where we remove cells.

```{r}
meox1_wildtype <- meox1_wildtype[, !(meox1_wildtype$nCount_RNA < 1000 | meox1_wildtype$nFeature_RNA < 1000)]
```
...
merged@meta.data %>% 
  ...
  summarise(., count = n()) %>%
    ...
    mutate(., RNA_snn_res.0.5_cluster_purity = count[is_endothlial == "Pure"]/sum(count)) %>% 
...
umap_features <- c(
  ...
  "pcna",
  "hand2",
  "fn1a") # not all, just the endo ones
```


```{r, fig.align='center', fig.height=20, fig.width=20}
FeaturePlot(merged, features = umap_features, ncol = 4)
```
...
DimPlot(merged, group.by = "RNA_snn_res.0.5", label = T)
```

```{r, fig.align='center', fig.height=5, fig.width=10}
run_dotplot(merged, dotplot.level.01, group = "RNA_snn_res.0.5")
...
run_dotplot_mid_purity <- function(seurat, genes, limits){
  ...
  DotPlot(seurat, features = genes, group.by = "RNA_snn_res.0.5", idents = clusters)  +plot_sizing_seurat + rotate_x
}
...
```{r}
merged$L1_celltype <- case_when(
  merged$RNA_snn_res.0.5 == 10 ~ "hippo+_VEC",
  merged$RNA_snn_res.0.5 %in% c(1,7,8) ~ "AEC",
  merged$RNA_snn_res.0.5 == 2 ~ "LEC",
  merged$RNA_snn_res.0.5 ==  3~ "VEC",
  merged$RNA_snn_res.0.5 == 0 ~ "cycling_EC",
  merged$RNA_snn_res.0.5 == 4 ~ "Fibroblast",
  merged$RNA_snn_res.0.5 == 9 ~ "Endocardium",
  merged$RNA_snn_res.0.5 %in% c(6,12) ~ "Neuronal",
  merged$RNA_snn_res.0.5 %in% c(5, 18, 17) ~ "Epithelial", 
  merged$RNA_snn_res.0.5 %in% c(11,19) ~ "Hematopoietic",
  merged$RNA_snn_res.0.5 %in% c(12, 13) ~ "Chromatophore",
  merged$RNA_snn_res.0.5 == 15 ~ "Epithelium_Fibroblast",
  merged$RNA_snn_res.0.5 == 16 ~ "Immune",
  merged$RNA_snn_res.0.5 == 14 ~ "Eye_lens"
)
...
merged <- add_seurat_names(
  seurat = merged,
  group = "RNA_snn_res.0.5",
  names = L1_seurat_names)
...
run_celltype_umaps <-function(seurat){
  colours <- Polychrome::dark.colors(length(merged$L1_celltype)) %>% unname()
  DimPlot(seurat, group.by = "L1_celltype", cols = colours, label = T) + ggtitle(seurat@misc$name) +
...
```{r,fig.align='center', fig.height=5, fig.width=8}
run_celltype_umaps(merged)
```

Stacked barplot for celltype vs batch

```{r, fig.align='center', fig.height=7, fig.width=10}
merged@meta.data %>% 
  group_by(L1_celltype, batch) %>% 
  summarise(., count = n()) %>% 
  ggplot(., aes(x = L1_celltype, y = count, fill = batch)) +
  geom_bar(position = "fill", stat = "identity") +
  plot_sizing + rotate_x +
  ggtitle("Batch effect in celltypes")
```
...
file_name <- here('output/data/Datsets/merged_wildtypes/merged_wildtypes_Level_01_stringent_annotated.qs2')
