...
Making Level 03 object for yap1 dataset, using stringent meox1 thresholds. 
...
Made here: ./.pre_processing_combining_wildtypes_dataset_makeObject_Level02_stringent_annotation.Rmd

```{r}
level_02 <- qs_read(here('output/data/Datsets/merged_wildtypes/merged_wildtypes_Level_02_stringent_annotated.qs2'))
```
...
Subset Level 01 to only include LEC and VEC subsets. Annotation was done in .pre_processing_combining_wildtypes_dataset_makeObject_Level02_stringent_annotation.Rmd
...
plot_umap_dataset <- function(seurat){
  ...
  plot_list[[3]] <- DimPlot(seurat,  group.by = c("RNA_snn_res.0.4"), label = T) + ggtitle(seurat@project.name)
...
Make for seurat clusters. 

```{r, fig.align='center', fig.height=5, fig.width=10}
level_03@meta.data %>% 
    select(., batch, RNA_snn_res.0.4) %>% 
    group_by(RNA_snn_res.0.4, batch) %>% 
    summarise(., count = n()) %>% 
    ggplot(., aes(x= RNA_snn_res.0.4, y = count, fill= batch)) +
    geom_bar(position = "fill", stat = "identity") +
    plot_sizing + ggtitle("RNA_snn_res.0.4 composition") +
...
run_dotplot(seurat = level_03, genes = level_03_dotplot, group = "RNA_snn_res.0.4")
```
...
```{r}
subset_osr2_pos <- subset(level_03, subset = osr2 > 0)
...
After cross checking annotations at 0.4 and 0.2, I think we can stick with 0.2

````{r, fig.align='center', fig.height=4, fig.width=8}
run_dotplot(seurat = level_03, genes = level_03_dotplot, group = "RNA_snn_res.0.2")
````
...
```{r}
level_03$L3_celltype <- case_when(
  level_03$RNA_snn_res.0.2 == 0 ~ "msVEC",
  level_03$RNA_snn_res.0.2 == 1 ~ "LEC",
  level_03$RNA_snn_res.0.2 == 2 ~ "hmsVEC",
  level_03$RNA_snn_res.0.2 == 3 ~ "cvpVEC"
)


# add different names
level_03$L3_celltype_descriptive <- case_when(
  level_03$RNA_snn_res.0.2 == 0 ~ "cxcl12a_VEC",
  level_03$RNA_snn_res.0.2 == 1 ~ "LEC",
  level_03$RNA_snn_res.0.2 == 2 ~ "hippo_VEC",
  level_03$RNA_snn_res.0.2 == 3 ~ "cvpVEC"
)
...
run_celltype_umaps <-function(seurat){
  colours <- Polychrome::dark.colors(length(unique(seurat$L3_celltype))) %>% unname()
  DimPlot(seurat, group.by = "L3_celltype", cols = colours) + ggtitle(seurat@misc$name)
}

```

```{r,fig.align='center', fig.height=5, fig.width=7}
run_celltype_umaps(level_03)
```
...
file_name <- here('output/data/Datsets/merged_wildtypes/merged_wildtypes_Level_03_stringent.qs2')
