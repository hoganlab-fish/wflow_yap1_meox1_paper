...
clusters_to_keep <- level_01@meta.data %>% 
  ...
  filter(., RNA_snn_res.0.2_cluster_purity > 0.95) %>% 
  pull(RNA_snn_res.0.2) %>% unique()

Idents(level_01) <- "RNA_snn_res.0.2"
...
Make UMAP for res 0.5

```{r, fig.align='center', fig.height=5, fig.width=7}
 DimPlot(level_02, group.by = "RNA_snn_res.0.5", shuffle = T, label = T)
```
...
run_dotplot(seurat = level_02, group = "RNA_snn_res.0.5", genes = level_02_dotplot) + ylab('') + xlab("")
```
...
level_02$L2_celltype <- case_when(
  level_02$RNA_snn_res.0.5 %in% c(1,4) ~ "VEC",
  level_02$RNA_snn_res.0.5 %in% c(6,8,9,10,11) ~ "AEC",
  level_02$RNA_snn_res.0.5 == 2 ~ "cycling_LEC_VEC",
  level_02$RNA_snn_res.0.5 == 3 ~ "cycling_AEC",
  level_02$RNA_snn_res.0.5 == 0 ~ "mixed_EC",
  level_02$RNA_snn_res.0.5 == 5 ~ "Endocardium",
  level_02$RNA_snn_res.0.5 == 7 ~ "LEC"
)
...
run_celltype_umaps <-function(seurat){
  colours <- Polychrome::dark.colors(7); names(colours) <- c("AEC", "VEC", "LEC", 
                                                             "cycling_LEC_VEC", "cycling_AEC", "Endocardium", "mixed_EC")
