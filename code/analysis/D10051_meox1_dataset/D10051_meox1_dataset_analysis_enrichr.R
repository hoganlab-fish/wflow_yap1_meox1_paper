...
# Load data ----
markers <- read_csv(here("output/analysis/D10051_meox1_dataset/Markers/Level_03_Markers_L3_celltype_fc0.00_minpct_0.01.csv"))
degs <- read_csv(here("output/analysis/D10051_meox1_dataset/DEGs/Level_03_DEG_mutVSwt_L3_celltype_fc0.00_minpct_0.01.csv"))

# Export path ----
save_dir <- here("output/analysis/D10051_meox1_dataset/enrichR")
...
filename <- here("output/analysis/D10051_meox1_dataset/enrichR/D10051_meox1_dataset_markers_L3_celltype_top100_enrichR.qs2")
...
degs_up_results_all <- lapply( c("LEC", "mVEC", "hmVEC"), function(celltype){
  print(celltype)
...
filename <- here("output/analysis/D10051_meox1_dataset/enrichR/D10051_meox1_dataset_dataset_DEGs_L3_celltype_mutVSwt_top100_UP_enrichR.qs2")
qs_save(object = degs_up_results_all, file = filename)

# Run for: DEGs (top 100 downregulated only) ----
degs_down_results_all <- lapply(c("LEC", "mVEC", "hmVEC"), function(celltype){
  input <- degs %>% filter(., p_val_adj < 0.05 & group == celltype & avg_log2FC < 0) %>%
    top_n(., n=100, wt = avg_log2FC) %>% pull(Gene)
  result <- run_enrichr(markers = input, databases = c('GO_Biological_Process_2018', 'GO_Biological_Process_AutoRIF', 'KEGG_2019', 'WikiPathways_2018'))
  result$cluster <- celltype
...
filename <- here("output/analysis/D10051_meox1_dataset/enrichR/D10051_meox1_dataset_DEGs_L3_celltype_mutVSwt_top100_DOWN_enrichR.qs2")
...
filename <- here("output/analysis/D10051_meox1_dataset/enrichR/D10051_meox1_dataset_markers_L3_celltype_top50_enrichR.qs2")
qs_save(object = markers_results_all, file = filename)
...
filename <- here("output/analysis/D10051_meox1_dataset/enrichR/D10051_meox1_dataset_DEGs_L3_celltype_mutVSwt_top50_UP_enrichR.qs2")
qs_save(object = degs_up_results_all, file = filename)
...
filename <- here("output/analysis/D10051_meox1_dataset/enrichR/D10051_meox1_dataset_DEGs_L3_celltype_mutVSwt_top50_DOWN_enrichR.qs2")
qs_save(object = degs_down_results_all, file = filename)
