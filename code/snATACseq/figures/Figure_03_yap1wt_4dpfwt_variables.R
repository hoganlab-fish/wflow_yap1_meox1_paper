...
source("../functions/AddModuleScore_atac.R")
...
figure.dir.out <- '/team_folders/hogan_lab/Hogan_Lab_Projects/yap1_project/scatacseq/Figures_yap_paper'; dir.create(figure.dir.out, showWarnings = F, recursive = T)
p4_limits <- c(27429000, 27430200)
p3_limits <- c(27425800, 27426500)
chr <- 'chr12'

# calculated here ../analysis/scatacseq/yap1_dataset/motif_scanning_all_redundant_231213.sh
...
fimo.results <- read.table(path.fimo.results, 
                           ...
                           header = TRUE)
# ---- get DAPs ----
# calculated here ../analysis/scatacseq/yap1_dataset/04_Yap1_Level_02_scatac_DAP.R
...
ClustersPhenotype_ordered_cols <- c(
  "#a1d99b",
  "#084594",
  "#ef3b2c", 
  "#fec44f",
...
names(ClustersPhenotype_ordered_cols) <- ClustersPhenotype_ordered
...
# ---- order meox1 peaks from chromatin accessibility ----
order.meox1.peaks <- c('12:27429569-27430069',
                       '12:27425914-27426414',
                       '12:27457097-27457597',
                       '12:27473119-27473619',
                       '12:27434654-27435154',
                       '12:27469741-27470241',
                       '12:27485071-27485571',
                       '12:27461927-27462427',
                       '12:27419091−27419591',
                       '12:27482293-27482793',
                       '12:27480571-27481071',
                       '12:27452929-27453429',
                       '12:27422847-27423347',
                       '12:27496735-27497235'
                       )

order.meox1.peaks.short.tead <- c('12:27425914-27426414',
                                  '12:27429569-27430069',
                                  '12:27461927-27462427',
                                  '12:27469741-27470241')


order.meox1.peaks.short.chrom <- c('chr12:27425914-27426414',
                                   'chr12:27429569-27430069',
                                   'chr12:27461927-27462427',
                                   'chr12:27469741-27470241')



# ################################################################################
# # ADD GENOTYPE/PHENOTYPE COLNAME FOR PROX1A LEVEL 02 only run once
# ################################################################################
# project.4dpf.level02@cellColData$Phenotype_Genotype <- paste(project.4dpf.level02@cellColData$ClustersPhenotype, project.4dpf.level02@cellColData$Sample, sep="_")
# 
# project.4dpf.level02 <- ArchR::saveArchRProject(
#   ArchRProj = project.4dpf.level02, load = TRUE
# )
#
# ################################################################################
# # GET TSS SET FOR YAP1 LEVEL02
# ################################################################################
# tss <- data.frame(getTSS(ArchRProj = project.yap1.level02))
# regions.df <- data.frame(getPeakSet(ArchRProj=project))
# meox1TX <- regions.df$nearestTSS[regions.df$nearestGene == 'meox1'] %>% unique()
# # meox1 is ENSDART00000105661
# tss[tss$tx_name == meox1TX, ] %>% View()
...
#                                name){
#   
#   markersPeaks.forplotting <- getMarkerFeatures(
#     ArchRProj = project, 
#     useMatrix = "PeakMatrix", 
#     groupBy = groupBy,
#     bias = c("TSSEnrichment", "log10(nFrags)"),
#     testMethod = "wilcoxon") #save?
#   
#   region <- getPeakSet(ArchRProj=project)
#   region_filtered <- region[region@elementMetadata@listData$nearestGene == 'meox1']
#   
...
#   # 
#   # #filter
#   # starts <- c(region_filtered_p3@ranges@start, region_filtered_p4@ranges@start)
#   # markersPeaks.forplotting.filtered <-markersPeaks.forplotting[markersPeaks.forplotting@elementMetadata@listData$start %in% starts & markersPeaks.forplotting@elementMetadata@listData$seqnames == chr] 
#   # rownames(markersPeaks.forplotting.filtered) <- paste(markersPeaks.forplotting.filtered@elementMetadata@listData$seqnames,paste(markersPeaks.forplotting.filtered@elementMetadata@listData$start, markersPeaks.forplotting.filtered@elementMetadata@listData$end, sep = "-"),sep = ":")
#   # 
#   # 
...
#   
#   
#   #only show p3 and p4 in LEC, VEC, AEC and Endocardium
#   region <- getPeakSet(ArchRProj=project)
#   region_filtered_p4 <- region[region@seqnames == "chr12" & region@ranges@start > p4_limits[1]  &  region@ranges@start<  p4_limits[2]]
#   region_filtered_p3 <- region[region@seqnames == "chr12" & region@ranges@start > p3_limits[1]  &  region@ranges@start< p3_limits[2]]
#   
