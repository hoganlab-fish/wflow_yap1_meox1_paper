...
################################################################################
# ---- load projects ----
path.project.yap1.level02 <- '/hogan_lab/Hogan_Lab_Projects/yap1_project/scatacseq/Dataset_Level_02_Endothelial_cells_resubset_PeaksPhenotype/'
path.project.yap1.level02_old <- '/hogan_lab/Hogan_Lab_Projects/yap1_project/scatacseq/Dataset_Level_02_Endothelial_cells_resubset_recalcPeaks'

#project.yap1.level02 <- copy_ArchR_Project(oldProjectPath = path.project.yap1.level02_old, newProjectPath = path.project.yap1.level02)
project.yap1.level02 <- loadArchRProject(path.project.yap1.level02)
################################################################################
# GET GENOME AND GENE ANNOTATION
################################################################################
macs2_path      <-  "/config/binaries/macs/2.2.7.1/bin/macs2"
...
################################################################################
# MAKE PSEUDOBULK REPLICATES with Phenotype_Level_02
################################################################################
project.yap1.level02 <- ArchR::addGroupCoverages(
  ArchRProj = project.yap1.level02, groupBy = "Phenotype_Level_02", force = TRUE
)

project.yap1.level02 <- ArchR::saveArchRProject(
  ArchRProj = project.yap1.level02, load = TRUE
)

################################################################################
# ADD PEAKSET FOR YAP1 LEVEL 02 only run once
################################################################################
project.yap1.level02 <- addReproduciblePeakSet(
...
# ---- Add peak matrix  ----
project.yap1.level02 <- ArchR::addPeakMatrix(project.yap1.level02, force = TRUE)
...
#save
plotName <- "MarkerPeaks_ccn2a_Phenotype.pdf"
